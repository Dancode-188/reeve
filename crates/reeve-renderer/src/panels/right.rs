...
fn render_span_detail(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    // While the live view owns the center panel, it owns this section
    // too: detail from the hidden loaded trace would describe a span
    // that is not on screen. Only Fleet shows the live tree; History
    // loads persisted detail here on Enter, and a mid-turn agent must
    // not mask it.
    if state.view_mode == crate::app::ViewMode::Fleet {
        if let Some(lv) = state.live_view_for_selected() {
