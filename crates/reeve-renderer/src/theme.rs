...
/// The shipped themes, in `T` cycling order. Names must match the `name`
/// field inside each embedded file.
...
impl Theme {
    /// Loads a shipped theme by name. The files are embedded so switching
